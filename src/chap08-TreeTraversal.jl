function walk!(curr::Union{NodeBT{T}, Nothing}, path::Vector{T}; search_order::String) where {T<:Number}
	if isnothing(curr) return nothing end
	if search_order == "pre" push!(path, curr.value) end
	walk!(curr.left, path; search_order=search_order)
	if search_order == "in" push!(path, curr.value) end
	walk!(curr.right, path; search_order=search_order)
...
function traverse_tree(root::NodeBT{T}; search_order::String)::Vector{T} where {T<:Number}
	...
	path = Vector{T}()
	walk!(root, path; search_order=search_order)
	return path
end
