...
function _walk!(curr::Union{NodeBT{T}, Nothing}, path::Vector{T}; search_order::String)::Nothing where {T<:Number}
...
function traverse_tree(root::NodeBT{T}; search_order::String)::Vector{T} where {T<:Number}
	if search_order ∉ ["pre", "in", "post"] error("oh no") end  # value checking here instead of in recursion
	path = T[]
